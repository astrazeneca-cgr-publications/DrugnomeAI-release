...
class Config:
	...
	def __init__(self, output_dir, config_file=None, verbose=False):
		self.config_dir_path = os.path.dirname(os.path.realpath(__file__))
		self.output_dir = output_dir
		self.verbose = verbose

		if config_file:
			self.disease_oriented = True
			print("Running disease oriented analysis\n")
		else:
			self.disease_oriented = False
			print("Running disease agnostic analysis \n")
	...
	def init_variables(self, list_delim=',\s+'):
		...
		self.data_source = self.conf['initial_settings']['data_source']

		# >> Mandatory config field
		if self.disease_oriented:
			required_field_error_msg = "\n[Error] Please provide 'Disease/Phenotype terms:' in input config file and re-run.\nSee also example config file (example-input/config.yaml) from drugnome_ai GitHub repo)."
			...
			self.seed_include_terms = self.conf['Disease/Phenotype terms']
			...
			self.phenotype = "disease agnostic"
		...
		self.druggability_feature_table = Path(self.compiled_data_dir / 'druggability_feature_table.tsv')
		self.filtered_by_disease_feature_table = Path(self.compiled_data_dir / 'filtered_by_disease_feature_table.tsv')
		self.processed_label_table = Path(self.compiled_data_dir / 'label_table.tsv')
