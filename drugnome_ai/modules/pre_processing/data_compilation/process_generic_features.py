...
class ProcessGenericFeatures:
    ...
    def process_exac(self, save_to_file=False):
        ...
        base_df = pd.read_csv(self.cfg.data_dir / 'exac-broadinstitute/all_genes_exac-broad_feature_table.txt',
                              sep='\t')
    ...
    def process_essential_mouse_genes(self, save_to_file=False):
        ...
        essential_mouse_df = pd.read_csv(self.cfg.data_dir / 'essential_mouse_genes/mouse_genes_essential.csv',
                                         index_col=False)
        essential_mouse_df.replace({'Y': 1, 'N': 0}, inplace=True)
        print(essential_mouse_df.shape)

        essential_mouse_df = essential_mouse_df[
            ['Gene_Name', 'Essential', 'Non-essential', 'Overexpressed_in_the_brain', 'GC%']]

        essential_mouse_df.columns = ['MouseGenes_' + x if x != 'Gene_Name' else x for x in essential_mouse_df.columns]

        if save_to_file:
            essential_mouse_df.to_csv(self.cfg.data_dir / 'essential_mouse_genes/compiled_essential_mouse_features.tsv',
                                      sep='\t', index=None)
    ...
    def process_gnomad(self, save_to_file=False):
        ...
        dupl_canonical_df['obs_sum'] = dupl_canonical_df['obs_lof'] + dupl_canonical_df['obs_mis'] + dupl_canonical_df[
            'obs_syn']
        ...
        uniq_gnomad_df.drop(
            ['transcript', 'canonical', 'obs_syn', 'exp_syn', 'oe_syn', 'oe_syn_lower', 'oe_syn_upper', 'syn_z',
             'gene_issues'], axis=1, inplace=True)
    ...
    def process_genic_intolerance_scores(self, save_to_file=False):
        ...
        rvis_exac_v2_df = pd.read_csv(self.cfg.data_dir / 'genic-intolerance/RVIS_Unpublished_ExACv2_March2017.txt',
                                      sep='\t')
        ...
        genic_intol_df = pd.merge(genic_intol_df, rvis_exac_v2_df, how='outer', left_on='Gene_Name',
                                  right_on='Gene_Name')
        print(genic_intol_df.shape)

        genic_intol_df.columns = ['GenicIntolerance_' + x if x != 'Gene_Name' else x for x in genic_intol_df.columns]

        if save_to_file:
            genic_intol_df.to_csv(self.cfg.data_dir / 'genic-intolerance/compiled_genic_intolerance_scores.tsv',
                                  sep='\t', index=None)

        return genic_intol_df

    def process_gwas_features(self, pattern_lists=None, search_term='All', verbose=False, signif_thres='5e-08',
                              save_to_file=False):
        ...
        if pattern_lists is None and save_to_file:
            gwas_df.to_csv(self.cfg.data_dir / ('gwas_catalog/' + search_term + '_genes_GWAS_features.tsv'), sep='\t',
                           index=None)
    ...
    def process_mgi_essential_features(self, save_to_file=False):
        ...
        has_lethal_pheno = query_human_pheno_df['High-level Mammalian Phenotype ID (space-delimited)'].apply(
            check_for_mp_ids)

        mgi_essential_df = pd.DataFrame(
            {'Gene_Name': query_human_pheno_df.loc[has_lethal_pheno, 'Human Marker Symbol'], 'MGI_essential_gene': 1})
        # de-duplicate
    ...
    def run_all(self):
        ...
        if hasattr(self.cfg, 'mantis_data_option'):
            ...
            generic_mantis_datasets.append(essential_mouse_df)
            print('Essential mouse genes:', essential_mouse_df.shape)

            gnomad_df = self.process_gnomad()
            generic_mantis_datasets.append(gnomad_df)
            print('Gnomad:', gnomad_df.shape)

            genic_intolerance_df = self.process_genic_intolerance_scores()
            generic_mantis_datasets.append(genic_intolerance_df)
            print('Genic Intolerance:', genic_intolerance_df.shape)

            gwas_features_df = self.process_gwas_features()
            generic_mantis_datasets.append(gwas_features_df)
            print('GWAS:', gwas_features_df.shape)

            mgi_essential_df = self.process_mgi_essential_features()
            generic_mantis_datasets.append(mgi_essential_df)
            print('mgi Essential:', mgi_essential_df.shape)

            print("\n>> Merging all data frames together...")
            mantis_features_df = reduce(lambda df1, df2: pd.merge(df1, df2, how='left', on='Gene_Name'), \
                                        generic_mantis_datasets)
            mantis_features_df.drop_duplicates(subset='Gene_Name', inplace=True)
            print('final generic mantis df shape: ', mantis_features_df.shape)
            mantis_features_df.to_csv(self.cfg.generic_feature_table, sep='\t', index=None)
            print("Saved to {0}".format(self.cfg.generic_feature_table))

        elif hasattr(self.cfg, 'genic_intol_option'):
            mantis_features_df = self.process_genic_intolerance_scores()
            mantis_features_df.drop_duplicates(subset='Gene_Name', inplace=True)
            print('Genic Intolerance shape:', mantis_features_df.shape)
            mantis_features_df.to_csv(self.cfg.generic_feature_table, sep='\t', index=None)
            print("Saved to {0}".format(self.cfg.generic_feature_table))

        else:
            print("Neither generic Mantis features nor Genic Intolerance Scores included")
