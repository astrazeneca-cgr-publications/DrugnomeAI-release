...
from drugnome_ai.modules.supervised_learn.core.ml_plot_functions import plot_feature_imp_for_classifier
...
class ClassifierEvaluator:
	...
	def process_gene_proba_predictions(self, top_hits=50, save_to_file=False, top_hits_to_save=1000,
		...
		self.percentile_df.columns = ['Gene_Name', 'drugnome_ai_proba']
		self.percentile_df['drugnome_ai_perc'] = 100 * self.percentile_df['drugnome_ai_proba'].rank(pct=True)
		...
		self.percentile_df.to_csv(self.cfg.superv_ranked_pred / (self.clf_id + '.drugnome_ai_predictions.csv'), index=False)
...
if __name__ == '__main__':
	...
	out_dir = sys.argv[1] #'../../config.yaml'
	cfg = Config(out_dir)
