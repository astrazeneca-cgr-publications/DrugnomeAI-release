...
def run(disease):
    ...
    base_dir = '../../../data/random_vs_seed-drugnome_ai_scores/' + disease
...
#     print(random_seed_genes[:10])
    ...
    real_df = pd.read_csv(base_dir + '/real_seeds.' + best_real_clf[disease] + '.All_genes.drugnome_ai_percentiles.csv', index_col=0)
    random_df = pd.read_csv(base_dir + '/random_seeds.' + best_rand_clf[disease] + '.All_genes.drugnome_ai_percentiles.csv', index_col=0)
    real_df.head()
...
#     print(known_random_perc.shape)
    ...
    sns.distplot(known_real_perc['drugnome_ai_perc'].tolist(), hist=False, kde=True,
                 color = 'darkblue',
                 kde_kws={'linewidth': 2}, label='real seed genes')

    sns.distplot(known_random_perc['drugnome_ai_perc'].tolist(), hist=False, kde=True,
                 color = 'orange',
                 kde_kws={'linewidth': 2}, label='random seed genes')


    res =  mannwhitneyu(known_real_perc['drugnome_ai_perc'], known_random_perc['drugnome_ai_perc'])
    print(res)
