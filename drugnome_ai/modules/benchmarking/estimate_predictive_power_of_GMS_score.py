...
merged_df['drugnome_ai_perc'] = 100 * merged_df['Proba'].rank(pct=True)
merged_df.head()
...
mantis_ml_score = 'drugnome_ai_perc' #Proba
...
plt.ylabel('drugnome_ai percentile scores')
plt.title('intol_score vs drugnome_ai correlation:' + str(intol_score_mantis_corr[0]))
# plt.show()
fig.savefig(cfg.superv_figs_out / 'intol_score_vs_drugnome_ai_scatterplot.pdf', bbox_inches='tight')
...
get_predictive_power_for_intol_score('drugnome_ai_perc')
