...
class CollapsingDrugnomeAIOverlap:
	...
	def calc_stepwise_hypergeometric(self, clf_str, all_clf, top_ratio, disease='GGE', pval_cutoff=0.05,
		...
		for analysis_type in analysis_types:
			...
			ax.set_xlabel('Top ' + str(round(100 * top_ratio, 1)) + '% drugnome_ai predicted genes', fontsize=14)
			...
			if not run_external_benchmark:
				...
				if analysis_type in primary_analysis_types:
					...
					merged_results_df.to_csv(Path(str(self.cfg.out_root / ('../../' + input_dir + '/Hypergeometric_results/drugnome_ai_vs_collapsing.Top_' + str(top_ratio) + '.' + method_identifier + '.' + disease + '.csv'))), index=False)
		...
		for analysis_type in analysis_types:
			if analysis_type in primary_analysis_types:
				...
				ax.set_title('['+ clf_str + '] Hypergeometric test: Collapsing analysis vs drugnome_ai predictions\n' + hypergeom_enrich_str, fontsize=20)
				break
...
if __name__ == '__main__':
	...
	overlap_obj = CollapsingDrugnomeAIOverlap(cfg)
