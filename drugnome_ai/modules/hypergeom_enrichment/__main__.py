...
from drugnome_ai.modules.hypergeom_enrichment.consensus_predictions import Consensus_Gene_Predictions
...
class ExternalRankingOverlap:
	...
	def calc_stepwise_hypergeometric(self, all_clf, pval_cutoff=0.05, collapsing_top_ratio=-1, 
		...
		print('drugnome_ai top genes:', len(mantis_ml_top_genes))
		...
		ax.set_xlabel('Top ' + str(round(100 * self.top_ratio, 1)) + '% drugnome_ai predicted genes', fontsize=14)
		...
		merged_results_df = pd.DataFrame({'Gene_Name': top_overlapping_genes, 'drugnome_ai_rank': mantis_ml_top_overlap_genes_ranking})
		...
		merged_results_df.sort_values(by='drugnome_ai_rank', ascending=True, inplace=True)
		merged_results_df.to_csv(self.enrichment_dir + '/drugnome_ai-vs-external_ranked_list.Top_' + str(self.top_ratio) + '.' + clf_alias[self.clf_str] + suffix_str + remove_seed_genes_str + '.csv', index=False)
...
def main():
	...
	parser.add_argument("-n", "--novel", action="count", required=False,
			    help="Run hypergeometric test against novel drugnome_ai predictions only")
