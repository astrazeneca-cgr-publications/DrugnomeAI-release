...
class Consensus_Gene_Predictions:
	...
	def plot_grouped_barplot(self, df, out_dir, width=0.1, relaxation=0):
		...
		ax.set_ylabel('drugnome_ai percentile score')
		title_str = 'drugnome_ai performance per classifier on the consensus "' + self.gene_class + '" genes from the enrichment test against external ranked gene list'
	...
	def find_consensus_from_selected_classifiers(self, clf_subset, df, relaxation=0):
		...
		for clf in clf_subset:
			tmp_mantis_ml_file = str(self.cfg.superv_ranked_pred / (clf + '.drugnome_ai_predictions.csv'))
			...
			tmp_mantis_df = tmp_mantis_df.rename(columns={'drugnome_ai_perc': clf})
			tmp_mantis_df.drop(['drugnome_ai_proba'], axis=1, inplace=True)
