...
def plot_oddsratio(values, keys, figure_name, model, log=False):
    ...
    plt.title('Enrichment of OMIM genes\nper rank interval (DrugnomeAI-'+model+')')
    plt.legend()
    plt.tight_layout()
    plt.show()
...
def read_data(model):
    ...
    fin = open('./All_OMIM_genes.txt','r')
    omim_genes = fin.readlines()
    omim_genes = [gene.strip() for gene in omim_genes]
    omim_genes_set = set(omim_genes)
    ...
    for x in range(1,len(inter)):
        ...
        c_genes = len(top_genes & omim_genes_set)
...
def draw_bargraph(model,ranks,clinic_genes):
    ...
    ax.set_title('Number of OMIM genes\nper rank interval (DrugnomeAI-'+model+')')
